...
class EdmTable(EdmObject):
    ...
    def __init__(
        ...
        _args: Dict = locals()
        super().__init__(obj_type="EdmTable")

        # Loop over every argument
        for attr, val in _args.items():
            setattr(self, f"__def_{attr}", val)
    ...
    def autofitDimensions(self, xborder: int = 10, yborder: int = 10) -> None:
        ...
        assert isinstance(self["__def_xborder"], int)
        assert isinstance(self["__def_yborder"], int)
        ...
        for ob in self.Objects:
            ...
            for axis_str, dim_str, list in [("x", "w", ws), ("y", "h", hs)]:
                ...
                border_val = self["__def_" + axis_str + "border"]
                assert isinstance(border_val, int)

                self_axis_str = self[axis_str]
    ...
    def __dimLists(self) -> Tuple[List[int], List[int]]:
        ...
        for ob in self.Objects:
            ...
            for axis_str, dim_str in [("x", "w"), ("y", "h")]:
                ...
                ob_dim_str = ob[dim_str]
                ob_table_axis_offset = ob["__EdmTable_" + axis_str + "off"]
                assert isinstance(ob_dim_str, int)
    ...
    def addObject(
        ...
        for attr, val in _args.items():
            ...
            if getattr(ob, attr) is not None:
                setattr(ob, f"__EdmTable_{attr}", val)
            else:
                setattr(ob, f"__EdmTable_{attr}", getattr(self, f"__def_{attr}"))
    ...
    def nextCell(self, max_y: int = -1) -> None:
        ...
        """
        assert isinstance(self["__def_y"], int)
    ...
    def nextCol(self) -> None:
        ...
        assert isinstance(self["__def_y"], int)
        assert isinstance(self["__def_x"], int)
        self["__def_y"] = 0
        self["__def_x"] += 1
