...
from common import embed, rd_visible, label, colour_changing_rd
...
class GBObject(object):
    ...
    def addScreen(self, filename, macros = "", embedded = False):
        self.screens.append(GBScreen(filename, macros, embedded))            
...
class GBScreen(object):    
    def __init__(self, filename, macros, embedded):
...
class GuiBuilder:
    def __init__(self, dom, errors = ERROR):
        ...
        require("dls.dependency_tree==1.4")
...
#        docnames,docpaths = tree.paths(globs=["/documentation"],include_name=True)        
        ...
        for node in self._elements(components):
            ...
            for ob in self._elements(node):
                typ = str(ob.nodeName)
                if typ in ["edm", "edmembed"]:                
                    if typ == "edmembed":
                        args = dict(embedded = True)
                    else:
                        args = dict(embedded = False)
    ...
    def get(self, name, glob = True):
        ...
        if len(ret) == 0:
            self.error("Get '%s' returned no results" % orig_name)
        return ret
    ...
    def component(self, name, desc, P, obs = [], filename = None, macrodict = {}, 
            preferEmbed = True, substituteEmbed = True, ar = None):
        ...
        if filename is None:
            ...
            screenobs = self.__screenObs(name, obs, preferEmbed)
            if screenobs:
...
#        component.update(macros) 
    ...
    def __screenObs(self, name, obs, preferEmbed = True):
        ...
        for ob in obs:
            ...
            rds = [s for s in ob.screens if not s.embedded]                                                
            # if preferEmbed then filter out rds
            if len(rds) > 0 and len(embeds) > 0:
                if preferEmbed:
                    rds = []                              
                else:
                    embeds = []
