...
def Summary(
    ...
    domain: str = "$(dom)",
    vtype: str = "temp",
    aspectratio: float = 0.65,
) -> EdmObject:
    """Take a list of dicts, and make a summary screen out of them.

    Each dict in row_dict should represent the line on a spreadsheet, ie
    have NAME, DESCRIPTION, NTEMP, NFLOW, NMOTOR, EDM_MACROS, FILE defined.
    ...
    Args:
        row_dicts (List[Dict[str, str]]): Dictionary representing a line from
            the spreadsheet.
        domain (str, optional): Beamline domain, e.g. BLxxI. Defaults to "$(dom)".
        vtype (str, optional): "temp", "flow", "motor" or "eloss". Defaults to "temp".
        aspectratio (float, optional): Aspect ration of the resulting screen.
            Defaults to 0.65.

    Returns:
        EdmObject: The returned screen object
    """
            ...
            if vtype != "flow" or not len(skip_list) == int(dict["NFLOW"]):
                ...
                if vtype == "motor":
                    ...
                    ob = shell_visible(
                        ...
                        20,
                        "Home " + dict["NAME"],
                        f'gnome-terminal --hide-menubar \
                            -e "$(bin)/$(dom)-motorhome.py {dict["NAME"]}"',
                    )
                    ...
                    xs = 90
                table.addObject(
                    rd(0, 0, xs, 20, dfilename, dict["EDM_MACROS"]), xoff=xoff
                )
