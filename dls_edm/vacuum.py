...
This script takes a signal list and creates the vacuum synoptic from the _GUI_VA
sheet."""
...
def gaugeRd(x,y,w,h,PREFIX,GID,GCTLR):
    # make a gauge object
    ob = rd(x,y,w,h,"mks937aGauge.edl","dom=$(dom), id="+GID)
    ob["displayFileName"][1]=quoteString("mks937a.edl")
    ob["symbols"][1]=quoteString("device="+PREFIX+GCTLR)
    ob["numDsps"] = 2
    return ob
                              
def Vacuum(row_dicts,title="BLxxI-VA",domain=None,flip_paths=None):
    """Create a vacuum screen from a row_dicts. row_dicts should be a list of dicts
    where each dict can contain the following values:
    ...
    obtained from the title. If flip_paths are defined, they should be a list of
    paths where the flipped symbols could be found. If filp_paths, the screen is
    ...
    for dict in row_dicts:
        ...
        if "RGA" in dict:
            ...
            ob.addObject( symbol(0,0,32,32,"rga-symbol.edl",PREFIX+RGA+":STA",\
                                 11) )
        ...
        if "PIRG" in dict:
            ...
            ob.addObject( symbol(0,0,32,32,"mks937aPirg-symbol.edl",\
                                 PREFIX+PIRG+":STA",17) )
            ob.addObject( label(36,0,60,16,PIRG,fontAlign=align) )
            ob.addObject( pressure(36,16,60,16,PREFIX+PIRG+":P",fontAlign=align) )
            table.addObject(ob)
            line_cons[x].append(ob)
        table.nextCell()        
        # add IMGs
        if "IMG" in dict:
            ...
            ob.addObject( gaugeRd(0,0,32,32,PREFIX,GID,GCTLR) )
            ob.addObject( symbol(0,0,32,32,"mks937aImg-symbol.edl",\
                                 PREFIX+IMG+":STA",17) )
        ...
        if "IONP" in dict:
            ...
            ob.addObject( rd(0,0,32,32,"digitelMpcIonpControl.edl",\
                             "device="+PREFIX+IONP) )
            ob.addObject( symbol(0,0,32,32,"digitelMpcIonp-symbol.edl",\
                                 PREFIX+IONP+":STA",10) )
            ob.addObject( label(36,0,60,16,IONP,fontAlign=align) )
            ob.addObject( pressure(36,16,60,16,PREFIX+IONP+":P",fontAlign=align) )
            table.addObject(ob)
            line_cons[x].append(ob)
        table.nextCell()
        # add WALL
    ...
    for ob in wall_obs:
        ...
        bottom.setPosition(x+w/2-5,y+h+5)
    ...
    for x in list(line_cons.keys()):
        ...
        if obs:
            ...
            line.setPosition(x,miny,move_objects=False)
    ...
    # create the spaces
    pairs = [ (spaces[i][0],spaces[i][1],spaces[i][2],spaces[i+1][2]) \
              for i in range(len(spaces)-1) ]
    for PREFIX,text,ob1,ob2 in pairs:
        (ob1x,ob1y),(ob2x,ob2y) = ob1.getPosition(),ob2.getPosition()
        (ob1w,ob1h),(ob2w,ob2h) = ob1.getDimensions(),ob2.getDimensions()
        ob = EdmObject("Group")
        ob.addObject( tooltip(ob1x+ob1w+2,ob1y+ob1h/2-4,ob2x-ob1x-ob1w-4,8,\
                              "Vacuum Space: "+text) )
        ob.addObject( rd(ob1x+ob1w+2,ob1y+ob1h/2-4,ob2x-ob1x-ob1w-4,8,\
                         "space.edl","device="+PREFIX+text) )
        ob.addObject( symbol(ob1x+ob1w,ob1y+ob1h/2-4,ob2x-ob1x-ob1w,8,\
                        "symbols-vacuum-symbol.edl",PREFIX+text+":STA",3,True) )
        ob.addObject( label(ob1x+ob1w+28,ob1y-4,80,16,text,fontAlign=align) )
        ob.addObject( pressure(ob1x+ob1w+20,ob1y+ob1h-12,80,16,\
                               PREFIX+text+":P",True,fontAlign=align) )
        screen.addObject(ob)
        
    # create screen
    hutchText = title
    if title.find("OH")>-1:
        hutchText = "Optics Hutch "+\
                    title[title.find("OH")+2].replace(".","-")
    if title.find("EH")>-1:
        hutchText = "Experiment Hutch "+\
                    title[title.find("EH")+2].replace(".","-")    
    if title.find("EE")>-1:
        hutchText = "Experimental Enclosure "+\
                    title[title.find("EE")+2].replace(".","-")      
    if title.find("BE")>-1:
        hutchText = "Branchline Enclosure "+\
                    title[title.find("BE")+2].replace(".","-")
    screen["title"] = quoteString(title.split(".")[0])
    if flip_paths:
        Flip_horizontal(screen,flip_paths,flip_group_contents=True) 
    Titlebar(screen,ta="VA",button="text",button_text=domain,header="text",\
             header_text=hutchText+" Vacuum Summary",\
             tooltip="generic-vacuum-tooltip",title=title.split(".")[0])
    return screen
...
#        if options.flip:
#            screen = Vacuum(subst_file.subst,filename,flip_paths=".")
#        else:
