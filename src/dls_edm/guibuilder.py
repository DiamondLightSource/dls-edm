...
class GuiBuilder:
    ...
    def summary(
        ...
        """Find all screen objects like srcFilename and display them in a summary screen.
        ...
        if group:
            ...
            # sort the objects by the parent name
            groupObjects = sorted(groupObjects, key=lambda obj: obj.name)
            ...
            for o in groupObjects:
                ...
                if objects:
                    ...
                    def _find_title_button() -> EdmObject:
                        match buttons:
                            # if empty...
                            case []:
                                # just make a label
                                title_button = label(0, 0, sobs_width, 20, o.name)
                            case [_]:
                                # if there is a single screen for this already, add a button for it
                                title_button = buttons[0]
                                title_button.setDimensions(sobs_width, 20)
                            case _:
                                # If there are multiple screens, search for the autogenerated one
                                button_groups: list[EdmObject] = [
                                    grps for grps in buttons
                                ]
                                for grp in button_groups:
                                    for obs in grp.Objects:
                                        displayFileName = obs.Properties[
                                            "displayFileName"
                                        ]
                                        assert isinstance(displayFileName, dict)
                                        if (
                                            # Make sure extra " are removed
                                            displayFileName[0].replace('"', "")
                                            == f"{o.name}.edl"
                                        ):
                                            title_button = grp
                                            title_button.setDimensions(sobs_width, 20)
                                            return title_button
                                        # if not found, use button 0
                                        title_button = buttons[0]
                                        title_button.setDimensions(sobs_width, 20)
                                    else:
                                        continue
                    ...
                    title_button = _find_title_button()
        ...
        if len(screen_objects) > 0 and len(extras) > 0:
            sobs = self.__screenObs(
                "",
                sorted(extras, key=lambda obj: obj.name),
                preferEmbed=False,
                preferTab=False,
            )
    ...
    def pmacSummary(self, pmacinfo: list[tuple[str, str, str, str, str, str]]):
        ...
        for step, step_detail, step_ioc, conn_type, step_ip, step_rack in pmacinfo:
            ...
            conn_type = conn_type if conn_type in conn_types else "tcpip"

            split_ip = step_ip.split(":", maxsplit=1)
            has_port = len(split_ip) == 2 and split_ip[1].isnumeric()

            match conn_type, has_port:
                case _, True:
                    step_ip, step_port = split_ip
                case "tcpip", False:
                    step_port = "1025"
                case "ssh", False:
                    step_port = "22"
